...
def _unpad_modernbert_input(
    ...
    if attention_mask.dim() != 2:
        ...
        unpadded_mask = torch.ones((batch_size, seq_len), device=inputs.device, dtype=torch.bool)
        seqlens_in_batch = unpadded_mask.sum(dim=-1, dtype=torch.int32)
        indices = torch.nonzero(unpadded_mask.flatten(), as_tuple=False).flatten()
    else:
...
class ModernBertModel(ModernBertPreTrainedModel):
    ...
    def _update_attention_mask(self, attention_mask: torch.Tensor, output_attentions: bool) -> tuple[
        Tensor, Tensor]:
        if output_attentions:
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if indices is None and cu_seqlens is None and max_seqlen is None:
                ...
                if attention_mask is None:
                    attention_mask = torch.ones((batch_size, seq_len), device=device, dtype=torch.bool)
                # unpad_attention_mask = attention_mask if attention_mask.dim()==2 else torch.ones((batch_size, seq_len), device=device, dtype=torch.bool)
                if inputs_embeds is None:
                    with torch.no_grad():
